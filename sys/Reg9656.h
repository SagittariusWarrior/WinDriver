...
//-----------------------------------------------------------------------------   
#define PCI9656_SRAM_SIZE                  (0x100000)
...
typedef struct _BAR0_REGS_
{
	unsigned char        Reserved1[0x2f];

	unsigned int         ob_size;

	unsigned char        Reserved2[0x1ff - (0x2f + 4)];

	struct
	{
		unsigned int         ob_offset_idx;
		unsigned int         ob_offset_hi;
	}ob_info[32];

	struct
	{
		unsigned int        ib_bar;
		unsigned int        ib_start_lo;
		unsigned int        ib_start_hi;
		unsigned int        ib_offset;
	}ib_info[4];

} BAR0_REGS, *PBAR0_REGS;
